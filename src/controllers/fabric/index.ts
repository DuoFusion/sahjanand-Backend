...
export const updateFabric = async (req, res) => {
    ...
    try {
        ...
        let isExist = await fabricModel.findOne({ name: body.name, isDeleted: false, _id: { $ne: new ObjectId(body.colorId) } })
        if (isExist) return res.status(404).json(new apiResponse(404, responseMessage.dataAlreadyExist("Name"), {}, {}));
        
        isExist = await fabricModel.findOne({ priority: body.priority, isDeleted: false, _id: { $ne: new ObjectId(body.colorId) } })
        if (isExist) return res.status(404).json(new apiResponse(404, responseMessage.dataAlreadyExist("Priority"), {}, {}));

        const response = await updateData(fabricModel, { _id: new ObjectId(body.colorId), isDeleted: false }, body, {});
